...
async function runTests() {
  ...
  const balanceResponse = await coordinator.processUserQuery('test-user-1', 'What is my current balance?');
  ...
  const historyResponse = await coordinator.processUserQuery('test-user-1', 'Show me my recent transactions');
  console.log('Response:', JSON.stringify(historyResponse, null, 2), '\n');

  // Test 3: Transfer Money
  console.log('Test 3: Transfer Money');
  const transferResponse = await coordinator.processUserQuery('test-user-1', 'Transfer $500 to savings');
  console.log('Response:', JSON.stringify(transferResponse, null, 2), '\n');

  // Test 4: Bill Payment
  console.log('Test 4: Bill Payment');
  const billResponse = await coordinator.processUserQuery('test-user-1', 'Pay $150 to Power Company');
  console.log('Response:', JSON.stringify(billResponse, null, 2), '\n');

  // Test 5: Financial Advice
  console.log('Test 5: Financial Advice');
  const adviceResponse = await coordinator.processUserQuery('test-user-1', 'How can I save more money?');
  console.log('Response:', JSON.stringify(adviceResponse, null, 2), '\n');

  // Test 6: Fraud Check
  console.log('Test 6: Fraud Check');
  const fraudResponse = await coordinator.processUserQuery('test-user-1', 'Check for suspicious activity');
